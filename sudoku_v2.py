...
# It Just Works!
ORIGIN_TEMPLATE_TABLE = [[(((x+y//3)+(y*3)) % 9)+1 for x in range(9)] for y in range(9)]
...
class CreateSudoku(object):
    ...
    def mix_horizontal(origin_table, first_line_to_swap=None, second_line_to_swap=None, horizontal_area_id=None):
        ...
        origin_table[first_line_in_table_to_swap], origin_table[second_line_in_table_to_swap] = \
            origin_table[second_line_in_table_to_swap], origin_table[first_line_in_table_to_swap]
        return origin_table

    def mix_vertical(self, origin_table, first_column_to_swap=None, second_column_to_swap=None, vertical_area_id=None):
        self.mirror_reverse(origin_table)
        self.mix_horizontal(origin_table, first_column_to_swap, second_column_to_swap, vertical_area_id)
        self.mirror_reverse(origin_table)
        return origin_table
    ...
    def mix_horizontal_block(origin_table, first_line_area_to_swap=None, second_line_area_to_swap=None, *args, **kwargs):
        for x in range(1, 4):
            origin_table[first_line_area_to_swap*3-x], origin_table[second_line_area_to_swap*3-x] = \
                origin_table[second_line_area_to_swap*3-x], origin_table[first_line_area_to_swap*3-x]
        return origin_table

    def mix_vertical_block(self, origin_table, first_column_area_to_swap=None, second_column_area_to_swap=None, *args, **kwargs):
        self.mirror_reverse(origin_table)
        self.mix_horizontal_block(origin_table, first_column_area_to_swap, second_column_area_to_swap)
        self.mirror_reverse(origin_table)
        return origin_table
    ...
    def reverse_horizontal(origin_table, *args, **kwargs):
        for x in range(len(origin_table)):
            origin_table[x][:] = origin_table[x][::-1]
        return origin_table

    @staticmethod
    def reverse_vertical(origin_table, *args, **kwargs):
        origin_table[:] = origin_table[::-1]
        return origin_table

    @staticmethod
    def mirror_reverse(origin_table, *args, **kwargs):
        for x in range(len(origin_table)):
            for y in range(len(origin_table)-x):
                origin_table[x][8-y], origin_table[8-y][x] = origin_table[8-y][x], origin_table[x][8-y]
        return origin_table
...
class PrintSudoku(object):
    ...
    def modified_secret_table(self, origin_table, difficult=DIFFICULT['1']):
        for x in range(len(origin_table)):
            for y in range(len(origin_table)):
                count_numbers_in_table = self.count_visible_numbers_in_table(origin_table)
                if count_numbers_in_table > difficult['max_nums_in_table']:
                    if difficult['min_nums_in_area'] != self.count_visible_numbers_in_area(origin_table, x, y):
                        random_remove_item_choise = random.randint(0, 10)
                        if not random_remove_item_choise:
                            origin_table[x][y] = 0
                elif count_numbers_in_table != difficult['min_nums_in_table']:
                    if difficult['min_nums_in_area'] != self.count_visible_numbers_in_area(origin_table, x, y):
                        random_remove_item_choise = random.randint(0, 50)
                        if not random_remove_item_choise:
                            origin_table[x][y] = 0
                else:
                    return origin_table
...
class TestingSudoku(CreateSudoku):
    ...
    def test_table(self, origin_table):
        for num in range(1, 10):
            error = self.check_line_for_repeat_numbers(origin_table, num) or \
                    self.check_column_for_repeat_numbers(origin_table, num) or \
                    self.check_areas_for_repeat_numbers(origin_table, num)
    ...
    def check_column_for_repeat_numbers(self, origin_table, num):
        self.mirror_reverse(origin_table)
        error = self.check_line_for_repeat_numbers(origin_table, num)
        self.mirror_reverse(origin_table)
        return error
    ...
    def check_areas_for_repeat_numbers(self, origin_table, num):
        for x in range(3):
            for y in range(3):
                error = self.check_area_for_repeat_numbers(origin_table, num, x, y)
                if error:
                    return True
        return False
...
if __name__ == '__main__':
    ...
    # new_print.format_and_print_table(new_table)
    # print("")
